...
{
       0,   124,   124,   131,   132,   133,   134,   136,   148,   149,
     150,   152,   152,   163,   163,   163,   163,   182,   187,   189,
     190,   191,   192,   193,   195,   200,   206,   211,   216,   222,
     223,   224,   225,   226,   227,   228,   229,   230,   231,   232,
     233,   234,   235,   236,   237,   238,   239,   240,   241,   242,
     243,   244,   245,   246,   248,   253,   277,   282,   297,   298,
     299,   301,   306,   310,   314,   318,   320,   321,   322,   324,
     324,   341,   343,   343,   352,   354,   355,   357,   365,   367,
     371,   375,   379,   383,   387,   391,   395,   399,   403,   405,
     406,   408,   412,   414,   415,   417,   418
};
#endif
...
#line 136 "sintatico.y"
        ...
        void *no = inserirHash(hashGlobal, (yyvsp[-4].token).valor, INT, 0);
        setTipo(no, VAR);
        setEhConstante(no);
        setAtrib(no, valor); 
...
#line 163 "sintatico.y"
        ...
        void *no = inserirHash(hashGlobal, (yyvsp[-11].token).valor, (yyvsp[-7].token).tipo, ptrCont);
        setTipo(no, FUNCTION);
...
#line 253 "sintatico.y"
        ...
        if((yyvsp[0].token).valor[1] == '\\'){
            ...
            if((yyvsp[0].token).valor[2] == 'n'){
                expr->atribuicao = '\n';
            }else if((yyvsp[0].token).valor[2] == 't'){
                expr->atribuicao = '\t';
            }else if((yyvsp[0].token).valor[2] == 'r'){
                expr->atribuicao = '\r';
            }else if((yyvsp[0].token).valor[2] == '0'){
                expr->atribuicao = '\0';
            }else if((yyvsp[0].token).valor[2] == '\\'){
                expr->atribuicao = '\\';
            }else if((yyvsp[0].token).valor[2] == '\''){
                expr->atribuicao = '\'';
            }else if((yyvsp[0].token).valor[2] == '\"'){
...
#line 324 "sintatico.y"
        ...
        Param *param = criarParamH((yyvsp[-4].token).tipo, (yyvsp[-7].token).valor, ptrCont, (yyvsp[0].param));
        if (!(yyvsp[-1].dim)){
            setTipo(no, VAR);
            param->tipoParam = VAR;
        }else{
