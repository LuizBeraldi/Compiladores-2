...
then
    DrawSkull
    echo -e "\n\nERRO: Arquivo regalloc.zip ***NAO*** encontrado!!!\n\n"
    exit
else
...
cp ./regalloc.zip ./safe_box/
if [ ! -e ./safe_box/regalloc.zip ];
then
    DrawSkull
    echo "Erro ao copiar arquivo regalloc.zip para pasta safe_box!!!"
    exit
fi
echo "Descompactando..."
unzip -j -o -q ./safe_box/regalloc.zip -d ./safe_box/
...
if [ $erro == 0 ];
then
    if [ ! -e ./safe_box/regalloc.exe ]; then
        if [ ! -e ./safe_box/regalloc ]; then
           DrawSkull
...
then
    ...
    do
        ...
        diff ./saidas/saida_$name ./saidas_padrao/"saida_"$name > /dev/null
        rc=$?;
        if [ $rc == 0 ]; 
        then 
            echo -e " -> \e[92mOK\e[39m"
